...
//Plasmablade
/obj/item/melee/plasmablade
	name = "captain's plasmablade"
	desc = "a bright pink plasmablade that would cut you if you were to touch it"
	icon_state = "plasmablade"
	inhand_icon_state = "sabre"
	lefthand_file = 'icons/mob/inhands/weapons/swords_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/weapons/swords_righthand.dmi'
	flags_1 = CONDUCT_1
...
/obj/item/melee/plasmablade/Initialize()
	. = ..()
	AddComponent(/datum/component/butchering, 30, 95, 5) //fast and effective, but as a sword, it might damage the results.

//Rapier Sheath

/obj/item/storage/belt/plasmablade
	name = "plasmablade sheath"
	desc = "An ornate sheath designed to hold a special plasmablade. It's fitted with an igniter."
	icon_state = "psheath"
	inhand_icon_state = "psheath"
	worn_icon_state = "ksheath"
	w_class = WEIGHT_CLASS_BULKY

/obj/item/storage/belt/plasmablade/ComponentInitialize()
	. = ..()
	AddElement(/datum/element/update_icon_updates_onmob)
	var/datum/component/storage/STR = GetComponent(/datum/component/storage)
	STR.max_items = 1
...
/obj/item/storage/belt/plasmablade/examine(mob/user)
	. = ..()
	if(length(contents))
		. += "<span class='notice'>Alt-click it to quickly draw the blade.</span>"

/obj/item/storage/belt/plasmablade/AltClick(mob/user)
	if(!user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, TRUE))
		return
	if(length(contents))
		var/obj/item/I = contents[1]
...
/obj/item/storage/belt/plasmablade/update_icon_state()
	icon_state = initial(inhand_icon_state)
	inhand_icon_state = initial(inhand_icon_state)
	worn_icon_state = initial(worn_icon_state)
	if(contents.len)
		icon_state += "-plasmablade"
		inhand_icon_state += "-plasmablade"
		worn_icon_state += "-katana"

/obj/item/storage/belt/plasmablade/PopulateContents()
	new /obj/item/melee/plasmablade(src)
	update_icon()
